...
async fn main() -> anyhow::Result<()> {
    ...
    let log_path_str = config.log_path.clone().unwrap_or("./".into());
    ...
    Server::new(acceptor)
        .serve(
            Service::new(router::router().hoop(set_metadata))
...
#[handler]
async fn set_metadata(depot: &mut Depot) {
    depot.inject(SERVER_CONFIG.metadata.clone());
}
